...
def main() -> None:
    ...
    # 2. Fallback for development: search near the python executable (venv bin)
    ...
    # 3. Fallback for development: search in target/debug relative to the source
    if not executable_path:
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
        potential_debug_bin = os.path.join(project_root, "target", "debug", bin_name)
        if os.path.exists(potential_debug_bin):
            executable_path = potential_debug_bin
